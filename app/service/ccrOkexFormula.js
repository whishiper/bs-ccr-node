...
*/
class CcrOkexFormulaService extends Service {
  /*
  @author:fsg
  ...
  */
  async trade_times({
    ...
    console.log('symbolBudget', symbolBudget);
    // 临时保存的均价
  ...
  */
  async buy_volume({ symbol, policy_series, trade_times, min_trade_amount }) {
    ...
    const fn = num => ('' + num).replace('0.', '').length;
    // console.log('amount_precision', amount_precision);
    if (!Array.isArray(policy_series)) {
      policy_series = JSON.parse(policy_series);
    }
    console.log(symbol, policy_series, trade_times, min_trade_amount);
    policy_series.forEach((item, index) => {
      // console.log(index, (trade_times * min_trade_amount * item));
  ...
  */
  async getSymbolCondition(symbol) {
    const { ctx, app } = this;
    const symbol_map = await ctx.service.ccrOkex.get_symbol_map();
    const targetSymbol = symbol_map[symbol];
    // 交易金额的精度
    const value_precision = targetSymbol.tick_size;
    // 交易对最小下单量
    const min_order_amt = targetSymbol.min_size;

    // 交易对报价的精度
    const price_precision = targetSymbol.tick_size;
    // 交易对基础币种计数精度
    const amount_precision = targetSymbol.size_increment;
  ...
  */
  async getDeepTotal({ symbol, length, type }) {
    ...
    const { data } = await ctx
      .curl(
        `${app.config.okexServer.url}/deep?symbol=${symbol}`,
        {
          method: 'GET',
          dataType: 'json',
        }
 ...
 */
  async store_split({ symbol, max_trade_order, sellPrice }) {
    ...
    const list = await app
      .curl(
        `${app.config.okexServer.url}/getRedisValueByKey?key=kline_${symbol}`,
        {
          method: 'GET',
          dataType: 'json',
        }
      ...
      .then(res => res.data);
    // console.log(`kline_${symbol} length`, Array.isArray(list), list.length);
    let str = '';
    list.forEach(item => {
    ...
    // console.log('store_split====res', res);

    const store_split = res.split(',')[1];
    // console.log('store_split', store_split);
    return store_split;
  }
  /*
  @author:fsg
...
module.exports = CcrOkexFormulaService;
