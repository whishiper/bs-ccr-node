...
@description:
*/
// key `zb-trade-condition_${signId}_${symbol}` 每个用户每个货币对的交易情况
class CcrZbService extends Service {
  constructor(ctx) {
    super(ctx);
  ...
  @params
  @description:处理货币对的 etc :OKB_ETH => okbeth
  */
  formatSymbol(symbol) {
    return symbol
      .split('_')
  ...
  @params
  @description: 转为zb格式 okbeth ==>  OKB-ETH
  */
  async formatSymbol2ZbType(symbol, custom_set_token = null) {
    const { ctx, app } = this;
    ...
    const obj = {
      formatZbSymbol: `${baseCoinName}_${valuationCoinName}`.toLocaleUpperCase(),
  ...
  async delete_api(query) {
    ...
    // app
    //   .curl(`${app.config.zbServer.url}/delKeysByKeyword`, {
    //     method: 'POST',
    //     dataType: 'json',
    //     data: {
    //       keyword: `zb-trade-condition_${signId}`,
    //     },
    //   })
    //   .then(res => res.data);
    return delRes;
  ...
  */
  async remove_api(query) {
    ...
    // console.log('zb remove_api query======', query);
    const errors = app.validator.validate(
      {
        // sign: {
        //   required: true,
    ...
    const res = await ctx.service.aliyunApiGateway
      ...
      .then(res => filterJavaResponse(ctx, res));
    // console.log('zb del trade_platform_api_bind_product_combo', res);
    return res;
  }
  /*
  @author:fsg
  ...
  */
  async bind_api(query) {
    const { ctx, app } = this;
    // console.log('zb bind_api query======', query);
    const errors = app.validator.validate(
      {
        sign: {
          required: true,
    ...
    const res = await this.aliyunApiGateway
      ...
      .then(res => filterJavaResponse(ctx, res));
    // console.log('zb trade_platform_api_bind_product_combo/binding', res);
    // TODO 将 redis 中zb-trade-condition_${spotInfo.id}_*中的secret都替换成当前secret @fsg 2020.3.30
    app
      .curl(`${app.config.zbServer.url}/replaceSecretByKeyword`, {
        method: 'POST',
        dataType: 'json',
        data: {
          keyword: `zb-trade-condition_${sign}`,
          secret,
        },
      })
      .then(res => res.data);
  ...
  */
  async verify_api(query) {
    ...
    const accountRes = await ctx
      .curl(`${app.config.zbServer.url}/verify_zb_api`, {
        method: 'POST',
        dataType: 'json',
        data: {
          secret,
      ...
      .then(res => res.data);
    // .then(filterZbResponse);
    console.log('zb accountRes===', accountRes);
    if (
      Reflect.has(accountRes, 'err_code') ||
      Reflect.has(accountRes, 'errors')
    ) {
      const { err_code, errors } = accountRes;
      console.log(err_code, errors);
      // 无效的参数
      if ([ 3005 ].includes(err_code)) {
        ctx.service.handleErrors.throw_error(errors);
      }
    } else {
      const {
  ...
  */
  ...
  @params
  @description:获取zb服务器的所有货币对数据
  */
  async get_symbol_map() {
    const { app } = this;
    const flag = await app.redis.get('internal').exists('zb_symbol_map');
    if (flag) {
      return JSON.parse(await app.redis.get('internal').get('zb_symbol_map'));
    }
    const data = await app
      .curl(`${app.config.zbServer.url}/getRedisValueByKey?key=zb_symbol_map`, {
        method: 'GET',
        dataType: 'json',
      })
      .then(res => res.data);
    await app.redis.get('internal').set('zb_symbol_map', JSON.stringify(data));
    return data;
  }
  async trigger_currency_kline(currency) {
    ...
    const res = await app
      .curl(
        `${app.config.zbServer.url}/trigger_currency_kline?currency=${currency}`,
        {
          method: 'GET',
          dataType: 'json',
        }
  ...
  */
  async initSymbol() {
    ...
    // console.log('data', symbol_map);
    const currencies = [ 'usdt', 'btc', 'qc' ];
    for (const item of currencies) {
      ...
      for (const v of arr) {
        ...
        ctx.service.aliyunApiGateway
          .index(
            `/trade_platform_coin_pair/?tradePlatformName=zb&isOfficialSet=1&isPopular=1&coinPairName=${v.symbol}`,
            'post',
            'default_handle_result'
          )
          .then(res => filterJavaResponse(ctx, res))
        ...
        ctx.service.aliyunApiGateway
          ...
          .then(r => {
            ...
            ctx.service.aliyunApiGateway
              .index(
                `/coin_sort/?coinId=${data}&tradePlatformName=zb&type=2`,
                'post',
                'default_handle_result'
              )
              .then(rep => filterJavaResponse(ctx, rep))
              // .then(rep => console.log(v, rep))
  ...
  */
  async initCurrencies() {
    ...
    for (const item of currencies) {
      ...
      const res1 = await ctx.service.aliyunApiGateway
        .index(
          `/coin_sort/?coinId=${data}&tradePlatformName=zb&type=1`,
          'post',
          'default_handle_result'
        )
        .then(res => filterJavaResponse(ctx, res));
  ...
  */
  async bindAllSymbols2Currency() {
    ...
    const _list = await app
      .curl(
        `${app.config.zbServer.url}/getRedisValueByKey?key=zb_symbol_list`,
        {
          method: 'GET',
          dataType: 'json',
        }
      ...
      .then(res => res.data);
    const zb_symbol_list = isJSON(_list) ? JSON.parse(_list) : _list;
    // console.log('zb_symbol_list', zb_symbol_list);
    try {
      for (const item of currencies) {
        const { id, name } = item;
        console.log('name', name, id);
        const arr = zb_symbol_list.filter(
        ...
        for (const v of arr) {
          ...
          // console.log('symbol_name', symbol_name, symbol_id);
  ...
  */
  async bindSymbol2Currency({ symbol_id, currency_id }) {
    ...
    ctx.service.aliyunApiGateway
      ...
      .catch(err => {
        // console.log('err', currency_id, symbol_id, err);
        // if (symbol_id - 0 > 600) {
        //   console.log('err', currency_id, symbol_id, err);
        // }
      });
  ...
  */
  async addSymbol(obj) {
    ...
    const res = await ctx.service.aliyunApiGateway
      ...
      .then(res => filterJavaResponse(ctx, res));
    console.log('coin_pair_choice post', res);
    if (!res) {
      ctx.service.handleErrors.throw_error(res);
    }
    const formatSymbolInfo = await this.formatSymbol2ZbType(symbol);
    console.log('formatSymbolInfo', symbol, formatSymbolInfo);
    symbol = formatSymbolInfo.formatZbSymbol;

    const redis_key = `zb-trade-condition_${signId}_${symbol}`;
    const redis_value = {
      tradePlatformApiBindProductComboId,
      plantFormName: 'zb',
      symbol,
    ...
    console.log('addSymbol', redis_value);
    // 提交java成功再添加到redis
    ...
    handleRedisData.zset.zadd.push({
      zsetName: `zb_${symbol}_zset`,
      value: 1,
      key: redis_key,
    });
    // 向国外egg的redis写入已选货币对
    handleRedisData.set.sadd.push({
      key: 'zb_choice_symbol_list',
      value: symbol,
    });
    // 手动将该货币对的k线数据写入一次，防止添加成功马上去一键设置但是此时国外egg服务器还未执行新的kline调度任务
    app.curl(
      `${app.config.zbServer.url}/trigger_symbol_kline?symbol=${symbol}`,
      {
        method: 'GET',
        dataType: 'json',
      }
    );
    app.curl(`${app.config.zbServer.url}/setRedis`, {
      method: 'POST',
      dataType: 'json',
  ...
  */
  async latestOpenPrice({ symbol }) {
    ...
    const { data } = await app.curl(
      `${app.config.zbServer.url}/latestOpenPrice?symbol=${symbol}`,
      {
        method: 'GET',
        dataType: 'json',
      }
  ...
  */
  async symbolInfo({ symbol, signId }) {
    const { ctx, app } = this;
    symbol = (await this.formatSymbol2ZbType(symbol)).formatZbSymbol;

    const redis_key = `zb-trade-condition_${signId}_${symbol}`;
    let target_symbol = await app
      .curl(
        `${app.config.zbServer.url}/getRedisValueByKey?key=${redis_key}&type=Hash`,
        {
          method: 'GET',
          dataType: 'json',
        }
    ...
    const java_key = `zb-trade-java_${signId}_${symbol}`;
    // 只有java操控的数据
    let extraData = await app
      .curl(
        `${app.config.zbServer.url}/getRedisValueByKey?key=${java_key}&type=Hash`,
        {
          method: 'GET',
          dataType: 'json',
        }
  ...
  */
  async currencyInfo({
    ...
    for (const item of list) {
      if (item.coinPair && item.coinPair.name) {
        const {
          formatZbSymbol,
          upper_base_currency,
        } = await this.formatSymbol2ZbType(
          item.coinPair.name,
          custom_set_token
        );
        const redis_key = `zb-trade-condition_${signId}_${formatZbSymbol}`;
        symbolRedisKeyList.push(redis_key);
        const symbolBalanceRes = ctx
          .curl(
            `${app.config.zbServer.url}/currency_account/${upper_base_currency}`,
            {
              method: 'POST',
              dataType: 'json',
              data: {
    ...
    // console.log('symbolRedisKeyList', symbolRedisKeyList);
    const symbolRedisKeyListPromise = app
      .curl(`${app.config.zbServer.url}/symbolList`, {
        method: 'POST',
        dataType: 'json',
        headers: {
          'content-type': 'application/json;charset=UTF-8',
    ...
    const balanceResPromise = ctx
      .curl(`${app.config.zbServer.url}/currency_account/${currency}`, {
        method: 'POST',
        dataType: 'json',
        data: {
          secret,
    ...
    // console.log('symbolBalanceList', symbolBalanceList);
    ...
    // console.log('zb redisValueList', redisValueList);
  ...
  */
  async setting(body) {
    ...
    for (const item of symbol_list) {
      ...
      item.symbol = (
        await this.formatSymbol2ZbType(item.symbol)
      ).formatZbSymbol;
      // 查出对应的策略详情
    ...
    try {
        ...
        const promise_item = new Promise(async (resolve, reject) => {
          try {
            ...
            if (!buyPrice || !sellPrice) {
              const latestPriceRes = await ctx.service.ccrZb.latestOpenPrice({
                symbol,
              });
              // 买价
            ...
            // 建仓间隔
            const store_split = await ctx.service.ccrZbFormula.store_split({
              symbol,
              sellPrice,
              max_trade_order,
            });
            ...
            // 交易倍数
            const trade_times = await ctx.service.ccrZbFormula.trade_times({
      ...
      for (const item of symbol_list) {
        ...
        const target_symbol = symbol_map[symbol];
        // 理论建仓价
        const theoreticalBuildPriceMap = await ctx.service.ccrZbFormula.averagePrice(
          {
            min_trade_amount: target_symbol.min_size,
        ...
        // console.log(`${item.symbol}理论建仓价`, theoreticalBuildPriceMap);
        // 当前货币对的拟交易情况
        const cur_symbol = purpose_trade_sition.find(v => v.symbol === symbol);
        // 计算每单买入量
        const buy_volume = await ctx.service.ccrZbFormula.buy_volume({
          symbol,
          policy_series,
        ...
        } = await ctx.service.ccrZbFormula.getSymbolCondition(symbol);
        // console.log(
        //   '交易对基础币种计数精度',
        //   amount_precision, //  交易对基础币种计数精度
        //   '交易对报价的精度',
        ...
        // }
        // console.log('buy_volume', buy_volume);
        // 计价货币
      ...
      // console.log('trade_times_err_arr', trade_times_err_arr);
  ...
  */
  async start_trade({
    ...
    const balanceRes = await ctx
      .curl(`${app.config.zbServer.url}/currency_account/${_quote_currency}`, {
        method: 'POST',
        dataType: 'json',
        data: {
          secret,
    ...
    const lastBuildPrice = await ctx.service.ccrZbFormula.lastBuildPrice({
      openPrice: buyPrice,
      max_trade_order,
      store_split,
    });
  ...
  */
  async forget_orders({ symbol, signId }) {
    ...
    // console.log(java_key, extraData);
    const res = await ctx.service.aliyunApiGateway
      .index('/coin_pair_choice/', 'put', 'default_handle_result', {
        coinPairId: symbol_id,
        id: coinPairChoiceId,
      ...
      .then(res => filterJavaResponse(ctx, res));
    console.log('forget_orders res', res);
    console.log('cur_groupId', cur_groupId);
    // 忘记订单 提交订单结算
    if (cur_groupId) {
    ...
    handleRedisData.hash.hdel.push({
      table: 'zb-not-finished-buy-order',
      field: redis_key,
    });
    app.curl(`${app.config.zbServer.url}/setRedis`, {
      method: 'POST',
  ...
  */
  async sell_all_orders({
    ...
    const sellRes = await ctx
      .curl(`${app.config.zbServer.url}/sell`, {
        method: 'POST',
        dataType: 'json',
        data: obj,
      })
    ...
    if (!sellRes || sellRes.name === 'Error') {
      ...
      ctx.service.handleErrors.throw_error(
        sellRes.errors.message || sellRes.errors
      );
    } else {
      const res = await ctx.service.aliyunApiGateway
        .index('/coin_pair_choice/', 'put', 'default_handle_result', {
        ...
        .then(res => filterJavaResponse(ctx, res));
      // console.log('sell_all_orders', res);
      return res;
    }
  }
  /*
  ...
  */
  async setTradeParams(body) {
    ...
    if (res) {
      ...
      const setRedisRes = await app.curl(`${app.config.zbServer.url}/setRedis`, {
        method: 'POST',
        dataType: 'json',
        data: handleRedisData,
        headers: {
