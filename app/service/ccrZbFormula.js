...
*/
class CcrZbFormulaService extends Service {
  /*
  @author:fsg
  ...
  */
  async getSymbolCondition(symbol) {
    ...
    // 交易对报价的精度
    const price_precision = targetSymbol.priceScale;
    // 交易对基础币种计数精度
    const amount_precision = targetSymbol.amountScale;
  ...
  */
  async getDeepTotal({ symbol, length, type }) {
    ...
    const { data } = await ctx
      .curl(
        `${app.config.zbServer.url}/deep?symbol=${symbol}`,
        {
          method: 'GET',
          dataType: 'json',
        }
 ...
 */
  async store_split({ symbol, max_trade_order, sellPrice }) {
    ...
    const list = await app
      .curl(
        `${app.config.zbServer.url}/getRedisValueByKey?key=kline_${symbol}`,
        {
          method: 'GET',
          dataType: 'json',
        }
...
module.exports = CcrZbFormulaService;
