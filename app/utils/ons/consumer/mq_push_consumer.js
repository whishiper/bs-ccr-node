...
const PullStatus = require("./pull_status");
...
class MQPushConsumer extends ClientConfig {
  constructor(options) {
    assert(
      ...
      "[MQPushConsumer] options.consumerGroup is required"
    );
    ...
    assert(
      mergedOptions.parallelConsumeLimit <= mergedOptions.pullBatchSize,
  ...
  get messageModel() {
    return this.options.isBroadcast
    ...
    if (this.messageModel === MessageModel.CLUSTERING) {
      ...
      this.subscribe(retryTopic, "*", async msg => {
        ...
        if (!MixAll.isRetryTopic(originTopic) && handler) {
          ...
          this.logger.warn(
            "[MQPushConsumer] retry message no handler, originTopic: %s, originMsgId: %s, msgId: %s",
            originTopic,
            originMsgId,
            msg.msgId
          );
  ...
  subscribe(topic, subExpression, handler) {
    ...
    assert(
      is.asyncFunction(handler),
      "[MQPushConsumer] handler should be a asyncFunction"
    );
    assert(
      !this.subscriptions.has(topic),
      `[MQPushConsumer] ONLY one handler allowed for topic=${topic}`
    );
    ...
    (async () => {
      try {
        ...
        while (!this._isClosed && this.subscriptions.has(topic)) {
          await this._consumeMessageLoop(
            topic,
            needFilter,
            tagsSet,
            subExpression
          ...
          while (pq.msgCount) {
            ...
            for (const msg of msgs) {
              ...
              if (!msg.tags || !needFilter || tagsSet.includes(msg.tags)) {
                ...
                this.logger.debug(
                  "[MQPushConsumer] message filter by tags=, msg.tags=%s",
                  subExpression,
                  msg.tags
                );
              }
  ...
  async consumeSingleMsg(handler, msg, mq, pq) {
    ...
    while (!this._isClosed) {
      if (msg.reconsumeTimes > this.options.maxReconsumeTimes) {
        this.logger.warn(
          "[MQPushConsumer] consume message failed, drop it for reconsumeTimes=%d and maxReconsumeTimes=%d, msgId: %s, originMsgId: %s",
          msg.reconsumeTimes,
      ...
      try {
        ...
        if (this.messageModel === MessageModel.CLUSTERING) {
          ...
          this.logger.warn(
            "[MQPushConsumer] BROADCASTING consume message failed, drop it, msgId: %s",
            msg.msgId
          );
          return;
        }
  ...
  pullMessageQueue(messageQueue) {
    ...
    (async () => {
      while (
        !_self._isClosed &&
        _self._processQueueTable.has(messageQueue.key)
      ) {
        try {
  ...
  async executePullRequestImmediately(messageQueue) {
    ...
    if (!subscriptionData) {
      this.logger.warn(
        "[mq:consumer] execute pull request, but subscriptionData not found, topic: %s, queueId: %s",
        messageQueue.topic,
    ...
    let commitOffset = 0;
    const subExpression = this.options.postSubscriptionWhenPull
      ? subscriptionData.subString
      : null;
    ...
    if (MessageModel.CLUSTERING === this.messageModel) {
      const offset = await this._offsetStore.readOffset(
    ...
    this.logger.info(
      "[MQPushConsumer] start to pull message from queue: %s, nextOffset: %s, commitOffset: %s, subExpression: %s, subVersion: %s",
      messageQueue.key,
    ...
    const pullResult = await this.pullKernelImpl(
      messageQueue,
      subExpression,
      subVersion,
      pullRequest.nextOffset,
    ...
    this.logger.info(
      "[MQPushConsumer] pull message result: %s from queue: %s, requestOffset: %s, nextBeginOffset: %s, minOffset: %s, maxOffset: %s, suggestWhichBrokerId: %s",
      pullResult.pullStatus,
      messageQueue.key,
      originOffset,
      pullResult.nextBeginOffset,
      pullResult.minOffset,
      pullResult.maxOffset,
      pullResult.suggestWhichBrokerId
    );
    ...
    switch (pullResult.pullStatus) {
      case PullStatus.FOUND: {
        ...
        this.logger.info(
          "[MQPushConsumer] pull message success, found new message size: %d, topic: %s, consumerGroup: %s, messageQueue: %s cost: %dms.",
          pullResult.msgFoundList.length,
          messageQueue.topic,
          this.consumerGroup,
          messageQueue.key,
          pullRT
      ...
      case PullStatus.NO_MATCHED_MSG:
        this.logger.debug(
          "[mq:consumer] no new message for topic: %s at message queue => %s",
          subscriptionData.topic,
          messageQueue.key
        );
      ...
      case PullStatus.OFFSET_ILLEGAL:
        this.logger.warn(
          "[mq:consumer] the pull request offset illegal, message queue => %s, the originOffset => %d, pullResult => %j",
          messageQueue.key,
          originOffset,
          pullResult
        );
  ...
  async pullKernelImpl(
    ...
    return await this._mqClient.pullMessage(
      result.brokerAddr,
      requestHeader,
      this.options.consumerTimeoutMillisWhenSuspend
    );
  }
  ...
  async findBrokerAddress(messageQueue) {
    let findBrokerResult = this._mqClient.findBrokerAddressInSubscribe(
      ...
      this.recalculatePullFromWhichNode(messageQueue),
      false
    );

    if (!findBrokerResult) {
      await this._mqClient.updateTopicRouteInfoFromNameServer(
        messageQueue.topic
      );
  ...
  recalculatePullFromWhichNode(messageQueue) {
    ...
    // }
    return (
      this._pullFromWhichNodeTable.get(messageQueue.key) || MixAll.MASTER_ID
    );
  }
  ...
  correctTagsOffset(pullRequest) {
    ...
    if (pullRequest.processQueue.msgCount === 0) {
      this._offsetStore.updateOffset(
        pullRequest.messageQueue,
        pullRequest.nextOffset,
        true
      );
  ...
  async rebalanceByTopic(topic) {
    this.logger.info(
      "[mq:consumer] rebalanceByTopic: %s, messageModel: %s",
      topic,
      this.messageModel
    );
    ...
    if (!mqSet || !mqSet.length) {
      this.logger.warn(
        "[mq:consumer] doRebalance, %s, but the topic[%s] not exist.",
        this.consumerGroup,
        topic
      );
      return;
    ...
    if (this.options.isBroadcast) {
      ...
      const cidAll = await this._mqClient.findConsumerIdList(
        topic,
        this.consumerGroup
      );
      this.logger.info(
        "[mq:consumer] rebalance topic: %s, with consumer ids: %j",
        topic,
        cidAll
      );
      if (cidAll && cidAll.length) {
        ...
        allocateResult = this.allocateMessageQueueStrategy.allocate(
          this.consumerGroup,
          this._mqClient.clientId,
          mqSet,
          cidAll
        );
        this.logger.info(
          "[mq:consumer] allocate queue for group: %s, clientId: %s, result: %j",
          this.consumerGroup,
          this._mqClient.clientId,
          allocateResult
        );
        changed = await this.updateProcessQueueTableInRebalance(
          topic,
          allocateResult
        );
      }
    ...
    if (changed) {
      this.logger.info(
        "[mq:consumer] do rebalance and message queue changed, topic: %s, mqSet: %j",
        topic,
        allocateResult
      );
  ...
  async updateProcessQueueTableInRebalance(topic, mqSet) {
    ...
    for (const key of this._processQueueTable.keys()) {
      ...
      if (topic === messageQueue.topic) {
        ...
        if (!mqSet.some(mq => mq.key === messageQueue.key)) {
          ...
          this.consumeType === ConsumeType.CONSUME_PASSIVELY
        ) {
          processQueue.droped = true;
          await this.removeProcessQueue(messageQueue);
          changed = true;
          this.logger.warn(
            "[MQPushConsumer] BUG doRebalance, %s, remove unnecessary mq=%s, because pull is pause, so try to fixed it",
            this.consumerGroup,
            messageQueue.key
          );
        }
    ...
    for (const messageQueue of mqSet) {
      ...
      if (nextOffset >= 0) {
        ...
        this.logger.info(
          "[mq:consumer] doRebalance, %s, add a new messageQueue, %j, its nextOffset: %s",
          this.consumerGroup,
          messageQueue,
          nextOffset
        );
        ...
        this.logger.warn(
          "[mq:consumer] doRebalance, %s, new messageQueue, %j, has invalid nextOffset: %s",
          this.consumerGroup,
          messageQueue,
          nextOffset
        );
  ...
  async computePullFromWhere(messageQueue) {
    try {
      const lastOffset = await this._offsetStore.readOffset(
        messageQueue,
        ReadOffsetType.READ_FROM_STORE
      );
      this.logger.info(
        '[mq:consumer] read lastOffset => %s from store, topic="%s", brokerName="%s", queueId="%s"',
        lastOffset,
        messageQueue.topic,
        messageQueue.brokerName,
        messageQueue.queueId
      ...
      switch (this.consumeFromWhere) {
        ...
        case ConsumeFromWhere.CONSUME_FROM_LAST_OFFSET:
          ...
          if (lastOffset >= 0) {
            ...
            // 重试队列则从队列头部开始
            if (
              messageQueue.topic.indexOf(MixAll.RETRY_GROUP_TOPIC_PREFIX) === 0
            ) {
              result = 0;
            } else {
              // 正常队列则从队列尾部开始
        ...
        case ConsumeFromWhere.CONSUME_FROM_TIMESTAMP:
          ...
          if (lastOffset >= 0) {
            ...
            const timestamp = utils
              .parseDate(this.options.consumeTimestamp)
              .getTime();
      ...
      this.logger.info(
        "[mq:consumer] computePullFromWhere() messageQueue => %s should read from offset: %s and lastOffset: %s",
        messageQueue.key,
        result,
        lastOffset
      );
      ...
      err.mesasge =
        "computePullFromWhere() occurred an exception, " + err.mesasge;
      this._handleError(err);
      return -1;
    }
  }
  ...
  async removeProcessQueue(messageQueue) {
    ...
    if (processQueue) {
      ...
      this.logger.info(
        "[mq:consumer] remove unnecessary messageQueue, %s, Droped: %s",
        messageQueue.key,
        processQueue.droped
      );
    }
  ...
  async sendMessageBack(msg, delayLevel, brokerName, consumerGroup) {
    const brokerAddr = brokerName
      ? this._mqClient.findBrokerAddressInPublish(brokerName)
      : msg.storeHost;
    const thatConsumerGroup = consumerGroup
      ? consumerGroup
      : this.consumerGroup;
    try {
      ...
      err.mesasge =
        "sendMessageBack() occurred an exception, " +
        thatConsumerGroup +
        ", " +
        err.mesasge;
      this._handleError(err);
      ...
      if (MixAll.isRetryTopic(msg.topic)) {
        ...
        newMsg = new Message(
          MixAll.getRetryTopic(thatConsumerGroup),
          "",
          msg.body
        );
        ...
        newMsg.properties[MessageConst.PROPERTY_MAX_RECONSUME_TIMES] = String(
          this.options.maxReconsumeTimes + 1
        );
      }

      newMsg.properties[MessageConst.PROPERTY_RECONSUME_TIME] = String(
